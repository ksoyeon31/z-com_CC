...
export default function BeforeLoginLayout({children,modal}: Props) { //뒤에 타입 쓰는거 너무 길고 복잡하다 싶으면 위의 코드처럼 변수로 만들어서 써도 됨
    return (
      <div className = {styles.container}>
        {/* css모듈의 역할이 가장 간단하면서도 서로 클래스 이름이 겹치지 않게 실제론 클래스 이름 옆에 랜덤의 영+숫자 조합을 모듈별로 다르게 붙여줘서
        클래스 이름이 같아도 겹치지 않음*/}
        {children}
        {/* 같은 레벨의 page는 children을 통해 보여지게 됨 */}
        {modal} 
        {/* @modal의 page는 modal을 통해 보여지게 됨 */}
