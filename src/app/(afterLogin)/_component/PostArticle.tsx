  ...
  return (
    <article onClickCapture={onClick} className={style.post}>
        {/* 클릭 이벤트와 a태그가 겹칠 때 이벤트 캡처링을 활용하면 좋음 
         ...
         {/* 에를들어 Div1(Div2(Div3)) 이렇게 Div1이 최상위 요소로 하위 요소들을 감싸고 있을때,
         이벤트 버블링은 Div3을 클릭시 콘솔을 출력하면 Div3>Div2>Div1 차례대로 찍히고,
         이벤트 캡처링은 Div3을 클릭시 콘솔에는 Div1>Div2>Div3 차례대로 찍힘(최상위 태그에서 이벤트가 발생한 해당 태그를 찾아서 내려감)  */}
      {children}
    </article>
  );
}